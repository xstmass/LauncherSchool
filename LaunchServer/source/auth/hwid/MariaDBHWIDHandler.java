...
public class MariaDBHWIDHandler extends HWIDHandler
{
    ...
    public MariaDBHWIDHandler(BlockConfigEntry block)
    {
        super(block);
        mySQLHolder = new MariaDBSourceConfig("hwidHandlerPool", block);

        // Read query
        query = VerifyHelper.verify(block.getEntryValue("query", StringConfigEntry.class),
                VerifyHelper.NOT_EMPTY, "MariaDB query can't be empty");
        ...
        querySelect = VerifyHelper.verify(block.getEntryValue("querySelect", StringConfigEntry.class),
                VerifyHelper.NOT_EMPTY, "MariaDB queryUpd can't be empty");
    ...
    public void writeHWID(HWID hwid, String username, Connection c)
    {
        ...
        try (PreparedStatement a = c.prepareStatement(queryUpd))
        {
            ...
            a.setQueryTimeout(MariaDBSourceConfig.TIMEOUT);
            a.executeUpdate();
        }
        catch (SQLException e)
        {
