...
public abstract class HWIDHandler extends ConfigObject implements AutoCloseable
{
    ...
    {
        Adapter<HWIDHandler> authHandlerAdapter = VerifyHelper.getMapValue(HW_HANDLERS, name, String.format("Unknown HWID handler: '%s'", name));
    ...
    public static void registerHandler(String name, Adapter<HWIDHandler> adapter)
    {
        ...
        VerifyHelper.putIfAbsent(HW_HANDLERS, name, Objects.requireNonNull(adapter, "adapter"), String.format("HWID handler has been already registered: '%s'", name));
    }
    ...
    public static void registerHandlers()
    {
        if (!registredHandl)
        {
            ...
            registerHandler("mysql-8", Mysql8HWIDHandler::new);
            registerHandler("mariadb", MariaDBHWIDHandler::new);
            registredHandl = true;
        }
    }
